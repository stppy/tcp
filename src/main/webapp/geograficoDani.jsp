...
$(document).ready(function(){
										...
										'<table class="table table-hover hitos">'+
											'<tbody>'+
												...
												'<tr><td colspan="2">'+
														...
														'<button type="submit" class="btn btn-success modalAgregarHito" parametros="'+institucionId+'-'+lineaAccionId+'-'+idDepartamento+'-'+idDistrito+'-'+accionId+'-'+institucionId+'-'+lineaAccionId+'-'+idDepartamento+'-'+idDistrito+'">Programar Hito</button>'+
														'<button type="submit" class="btn btn-success modalDeclararAvance" parametros="'+institucionId+'-'+lineaAccionId+'-'+idDepartamento+'-'+idDistrito+'-'+accionId+'-'+institucionId+'-'+lineaAccionId+'-'+idDepartamento+'-'+idDistrito+'" >Declarar Avance</button></td></tr>'+
									...
									'<div class="box box-danger">'+
		      					  	...
		      					  	'</div>'+
					                		...
					                		'<div class="row">'+
			      					    	...
			      					    	'</div>'+		// fin row		      					    	
			      					    	'</div>'+
			      					    	'<button type="button" class="btn btn-default modalVerificarDestinatarios" data-dismiss="modal">Destinatarios</button>'+
		    ...
		    this.producto = function(){
		    	...
		    	function jsonpCallbackProducto(data) {
			    	...
			    	function jsonpCallbackProductoUnidadMedida(data) {
				    	...
				    	$.ajax({
				         	 url:'/ajaxSelects?accion=getAsignacionPresiVersion&nivel=12&entidad=1&tipo='+linkTipoPrograma+'&programa='+linkPrograma+'&subPrograma='+linkSubPrograma+'&proyecto='+linkProyecto+'&producto='+linkProducto+'&anho=2016',
				          	type:'get',
				          	crossDomain: 'true',
				          	dataType:'jsonp',
			                jsonp: 'callback',
	...
	$("body").on("click", ".modalVerificarDestinatarios",function(event){
		event.stopPropagation();
		...
		//callback para traer Destinatarios de SPR
							   

					      	
						//callback para traer Departamentos de SPR
					      		 ...
					      		 var datosDestinatarios =[];
						      		$.ajax({
							        	url:'/ajaxSelects?accion=getDestinatarios',
							          	...
							          	dataType:'jsonp',
						                jsonp: 'callback',
						                jsonpCallback: 'jsonpCallbackDestinatario',
							          	async:false,
							          	success: function( data, textStatus, jqXHR) {
							          		if(data.success){
							          			jsonpCallbackDestinatario(data);
							          		}
							          	}
							        });
						      		
							      	function jsonpCallbackDestinatario(data) {
							      		datosDestinatarios= data;
							      		
							      		var datosProductosDestinatarios =[];
							      		$.ajax({
								        	url:'/ajaxSelects?accion=getProductoDestinatario&nivel='+linkNivel+'&entidad='+linkEntidad+'&tipoPresupuesto='+linkTipoPrograma+'&programa='+linkPrograma+'&subprograma='+linkSubPrograma+'&proyecto='+linkProyecto+'&producto='+linkProducto,
								          	type:'get',
								          	crossDomain: 'true',
								          	dataType:'jsonp',
							                jsonp: 'callback',
							                jsonpCallback: 'jsonpCallbackProductoDestinatario',
								          	async:false,
								          	success: function( data, textStatus, jqXHR) {
								          		if(data.success){
								          			jsonpCallbackProductoDestinatario(data);
								          		}
								          	}
								        });
							      		
							      		function jsonpCallbackProductoDestinatario(data) {
								      		datosProductosDestinatarios= data;
								      		
								      								      		
								      		
								      		modalVerificarDestinatarios+='<div class="modal fade modalVerificarDestinatarios" id="destinatario-'+linkNivel+'-'+linkEntidad+'-'+linkTipoPrograma+'-'+linkPrograma+'-'+linkSubPrograma+'-'+linkProyecto+'-'+linkProducto+'" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">'+
								    		  '<div class="modal-dialog modal-lg" >'+
								    		    '<div class="modal-content">'+
								    		      '<div class="modal-header">'+
								    		      '<button type="button" class="close modalHitoAvances" data-dismiss="modal" parametros="">&times;</button>'+
								    		      '</div>'+
								    		           ...
								    		           'Destinatarios'+
								    		            ...
								    		            '<div class="box-body scroll-x pre-scrollable">'+
								    	                  ...
								    	                  '<th>Destinatario</th>';
								    	                      ...
								    	                      '<br>'+
								    		                    '<div class="box-footer">'+
								    		                    	'<button type="submit" class="btn btn-warning" id="guardar-destinatarios-'+linkNivel+'-'+linkEntidad+'-'+linkTipoPrograma+'-'+linkPrograma+'-'+linkSubPrograma+'-'+linkProyecto+'-'+linkProducto+'">Guardar</button>'+
								    		                    	'<button type="button" class="btn btn-default modalHitoAvances" parametros="">Cerrar</button>'+
								    		                    '</div>'+
								    						'</form>'+
								    					'</ul>'+
								    				'</div>'+
								    			...
								    			'</div>';
								    	                      ...
								    	                      '<tbody class="table-body-producto-destinatario" id="tabla-destinatarios-'+linkNivel+'-'+linkEntidad+'-'+linkTipoPrograma+'-'+linkPrograma+'-'+linkSubPrograma+'-'+linkProyecto+'-'+linkProducto+'">';
												  	    	  for (var iDest=0 ;iDest<datosDestinatarios.length;iDest++){
													    		  cuerpoTablaDestinatario+="<tr><td>"+datosDestinatarios[iDest].nombre+"</td>";
														    	  if (datosProductosDestinatarios[iDest].length>0){  
														    		  for (var iDepto=0;iDepto<18;iDepto++){
														    			  cuerpoTablaDestinatario+='<td><input type="text" class= "destinatario-dest-dept" size="5px" name="destinatarios-'+producto+'-'+iDest+'-'+iDepto+'" id="dest-'+linkNivel+'-'+linkEntidad+'-'+linkTipoPrograma+'-'+linkPrograma+'-'+linkSubPrograma+'-'+linkProyecto+'-'+linkProducto+'-'+iDest+'-'+iDepto+'" value="'+datosProductosDestinatarios[iDest][iDepto].cantidad+'"></td>';
														    			  
														    		  }
														    		  iDepto=88; cuerpoTablaDestinatario+='<td><input type="text" class= "destinatario-dest-dept" size="5px" name="destinatarios-'+producto+'-'+iDest+'-'+iDepto+'" id="dest-'+linkNivel+'-'+linkEntidad+'-'+linkTipoPrograma+'-'+linkPrograma+'-'+linkSubPrograma+'-'+linkProyecto+'-'+linkProducto+'-'+iDest+'-'+iDepto+'" value="'+datosProductosDestinatarios[iDest][18].cantidad+'"></td>';
														    		  iDepto=99; cuerpoTablaDestinatario+='<td><input type="text" class= "destinatario-dest-dept" size="5px" name="destinatarios-'+producto+'-'+iDest+'-'+iDepto+'" id="dest-'+linkNivel+'-'+linkEntidad+'-'+linkTipoPrograma+'-'+linkPrograma+'-'+linkSubPrograma+'-'+linkProyecto+'-'+linkProducto+'-'+iDest+'-'+iDepto+'" value="'+datosProductosDestinatarios[iDest][19].cantidad+'"></td>';
														    	  }else{
														    		  for (var iDepto=0;iDepto<18;iDepto++){
														    			  cuerpoTablaDestinatario+='<td><input type="text" class= "destinatario-dest-dept" size="5px" name="destinatarios-'+producto+'-'+iDest+'-'+iDepto+'" id="dest-'+linkNivel+'-'+linkEntidad+'-'+linkTipoPrograma+'-'+linkPrograma+'-'+linkSubPrograma+'-'+linkProyecto+'-'+linkProducto+'-'+iDest+'-'+iDepto+'" value=""></td>';
														    			  
														    		  }
														    		  iDepto=88; cuerpoTablaDestinatario+='<td><input type="text" class= "destinatario-dest-dept" size="5px" name="destinatarios-'+producto+'-'+iDest+'-'+iDepto+'" id="dest-'+linkNivel+'-'+linkEntidad+'-'+linkTipoPrograma+'-'+linkPrograma+'-'+linkSubPrograma+'-'+linkProyecto+'-'+linkProducto+'-'+iDest+'-'+iDepto+'" value=""></td>';
														    		  iDepto=99; cuerpoTablaDestinatario+='<td><input type="text" class= "destinatario-dest-dept" size="5px" name="destinatarios-'+producto+'-'+iDest+'-'+iDepto+'" id="dest-'+linkNivel+'-'+linkEntidad+'-'+linkTipoPrograma+'-'+linkPrograma+'-'+linkSubPrograma+'-'+linkProyecto+'-'+linkProducto+'-'+iDest+'-'+iDepto+'" value=""></td>';
														    	  }
												  	    	 ...
												  	    	 $(tabla).append(cuerpoTablaDestinatario);
								    		    				...
								    		    				$(".modalVerificarDestinatarios").modal('show');
								      		
							      		}//fin callback para traer ProductoDestinatarios
  		
							      	};//fin callback para traer Destinatarios
					      		
					      	};//fin callback para traer Departamentos
	...
	});//fin modal para desplegar Destinatarios
