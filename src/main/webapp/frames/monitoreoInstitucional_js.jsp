<script>
	...
	function renderFlow(periodo,etiquetaSeleccionado){
		...
		var avances = $.ajax({
			url:'/tablero/ajaxSelects2?action=getPivotAvance&anho='+periodo+'&mes=01',
			type:'get',
			dataType:'json',
			async:false       
		}).responseText;
		...
		for (var i=1; i<avances.length;i++){
			if (fechaAnterior>avances[i].avanceFecha){
				...
				flowContent+='        </div>'+
		                ...
		                '            <a class="btn btn-primary btn-xs modalEvidencias" parametros="'+avances[i].laId+'-'+avances[i].Periodo+'" >Ver evidencias</a>'+
		                '        </div>'+
		                '   </div>'+
		                '</li>';
		...
		flowContent+='        </div>'+
		'        <div class="timeline-footer">'+
		'            <a class="btn btn-primary btn-xs modalEvidencias" parametros="'+avances[0].ilaId+'-'+avances[0].Periodo+'">Ver evidencias</a>'+
		'        </div>'+
		'   </div>'+
		'</li>'
	...
	function renderModalEvidencias(lineaAccionId, periodo) {
		...
		var webServicesEvidencia = $.ajax({
			url:'/tablero/ajaxSelects2?action=getEvidenciaAvanceLineaAccion&lineaAccionId='+lineaAccionId+'&anho='+periodo+'&mes=01',
		  	type:'get',
		  	dataType:'json',
		  	async:false
		}).responseText;
