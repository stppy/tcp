					...
					for(var i=0;i<institucion.length;i++){		
						...
						//var promedio = 0;
	 					...
	 					var desPaisInstjson = $.ajax({
	 						url:'http://spr.stp.gov.py/tablero/ajaxSelects2?action=getResumenLineasAccionProgramacionInstDptoDist&institucionId='+institucion[i].id,
					      	type:'get',
					      	dataType:'json',
					      	crossDomain:true,
					      	async:false       
						...
						if(desPaisInstAux[0] == 0){
							promedio = 0/0;
						}
						objeto.institucionId = institucion[i].id;
						objeto.promedio = desPaisInstAux[0];
...
/* 								var lineaAccionDepartamento = $.ajax({
							...
							for(var d = 0; d < distritos.length; d++){
								...
								for (var i = 0; i< entidades.length;i++){
									...
									var lineaAccionDepartamento = $.ajax({
								    	url:'http://spr.stp.gov.py/tablero/ajaxSelects2?action=getResumenLineasAccionProgramacionInstDptoDist&departamentoId='+e.target.feature.properties.dpto+'&institucionId='+entidades[i].institucion_id,
								      	type:'get',
								      	dataType:'json',
								      	crossDomain:true,
								      	async:false       
...
function renderTableroLineaAccion(institucionId,deptoId,distId){
	...
	for(var n=0; n<lineasProgramadas.length;n++){		
		if(lineasProgramadas[n].insLineaAccionId==linea_accion_id){
			...
			'<td>'+numeroConComa(lineasProgramadas[n-1].cantDestinatarioReal)+'</td>'+
			'<td>'+numeroConComa((lineasProgramadas[n-1].costoAc/1000000).toFixed(2))+'</td>'+
			'</tr>';
...
$("body").on("click", "#tablaInstituciones",function(event){
	...
	if (institucion_id==""){
		...
		if(institucion_id != "" && depto_id != ""){
			var a = renderTableroLineaAccion(institucion_id,depto_id);
			$("#cuerpoTableroLineaAccion").html(a);
		}
		if(institucion_id != "" && depto_id != "" && dist_id != ""){
			var a = renderTableroLineaAccion(institucion_id,depto_id,dist_id);
			$("#cuerpoTableroLineaAccion").html(a);
		}
